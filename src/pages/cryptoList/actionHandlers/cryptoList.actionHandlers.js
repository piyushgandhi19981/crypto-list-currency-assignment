...
import { getResolvedList, getUpdatedRecentSearches } from "../helpers/cryptoList.helpers";
...
const handleInit = async ({ setState, state }) => {
  ...
  const { listData, tableData } = getResolvedList({ page, rows, totalData }, searchText);
...
const handleFilterChange = async (params, updatedCurrency) => {
  ...
  const totalData = await fetchCryptoDetailsAction({ currency: updatedCurrency });
  await localStorage.setItem(LS_KEYS.CURRENCY, updatedCurrency);
  const { rows, searchText } = state;
  const { listData, tableData } = getResolvedList({ page: 1, rows, totalData }, searchText);
  setState({ totalData, data: tableData, isLoading: false, listData, page: 1, currency: updatedCurrency });
};
...
const handleSearchChange = async (params, searchText) => {
  ...
  if(searchText){
    updatedRecentSearches = getUpdatedRecentSearches(recentSearches, searchText)
    await localStorage.setItem(LS_KEYS.RECENT_SEARCHES, JSON.stringify({ recentSearches: updatedRecentSearches }));
  }
  const { listData, tableData } = getResolvedList({ page: 1, rows, totalData }, searchText);
  setState({ data: tableData, listData, recentSearches: updatedRecentSearches ? updatedRecentSearches : recentSearches, searchText, page: 1 })
};
